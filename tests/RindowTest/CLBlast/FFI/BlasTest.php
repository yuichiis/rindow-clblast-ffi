...
class BlasTest extends TestCase
{
    ...
    {
        return $this->hostBufferFactory()->Buffer($size,$dtype);
    }
    ...
    public function testScalFullRange()
    {
        $blas = $this->getBlas();
        $NMITEM = 2048;
        $X = $this->array(range(0,$NMITEM-1),dtype:NDArray::float32);
        [$N,$alpha,$XX,$offX,$incX,$queue,$events] =
            $this->translate_scal(2,$X);
        $blas->scal($N,$alpha,$XX,$offX,$incX,$queue,$events);
        $events->wait();
        $trues = array_map(fn($x)=>$x*2 , range(0,$NMITEM-1));
    ...
    public function testScalOffsetRange()
    {
        ...
        $events->wait();
        $trues = array_map(fn($x)=>($x>=intdiv($NMITEM,2))?$x*2:$x , range(0,$NMITEM-1));
        $this->assertEquals($trues,$X->toArray());
    }
    ...
    public function testNrm2Normal()
    {
        ...
        // float32
        $dtype = NDArray::float32;
        $X = $this->array([[1,2],[3,4]],dtype:$dtype);
        $R = $this->zeros([],dtype:$dtype);
        [$N,$RR,$offR,$XX,$offX,$incX,$queue,$events] =
            $this->translate_amin($X,output:$R);
        ...
        // complex64
        $dtype = NDArray::complex64;
        $X = $this->array($this->toComplex([[1,2],[3,4]]),dtype:$dtype);
        $R = $this->zeros([],dtype:$dtype);
        [$N,$RR,$offR,$XX,$offX,$incX,$queue,$events] =
            $this->translate_amin($X,output:$R);
    ...
    */
    ...
    public function testTrsvNormal($params)
    {
        ...
        $this->assertTrue($this->isclose($trues,$X,rtol:1e-3));
    ...
    public function testTrsvComplexTranspose($params)
    {
        ...
        $events->wait();
        $trues = $this->array([C(7,9),C(8,8),C(9,7)],dtype:$dtype);
        $this->assertTrue($this->isclose($trues,$X,rtol:1e-2));
    ...
    {
        $this->markTestSkipped("clBlast not support ConjNoTrans");
        return;
